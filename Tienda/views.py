...
def Listar_fundaciones(request):
    fundaciones=Refugios3.objects.all()
    page=request.get.get('page',1)
    try:
        paginator=Paginator(fundaciones,5)
        fundaciones=paginator.page(page)
    except:
        raise Http404
    data={'entity':fundaciones,
            'paginator':paginator}
    return render(request, 'Tienda/producto/Modificar_fundacion.html',data)
# Modificar------------------------------------------------------------------------------------------
